...
@scenario(
    "features/glaze_rendering.feature", "Passing test renders as a single compact line"
)
def test_passing_compact(): ...


@scenario(
    "features/glaze_rendering.feature", "Multiple passing tests render one line each"
)
def test_multiple_passing(): ...


@scenario(
    "features/glaze_rendering.feature", "Failing test renders inline assertion error"
)
def test_failing_inline(): ...


@scenario(
    "features/glaze_rendering.feature",
    "Failing test never defers its error to a separate block",
)
def test_failing_not_deferred(): ...


@scenario(
    "features/glaze_rendering.feature", "Class-based tests render with a class header"
)
...
@scenario(
    "features/glaze_rendering.feature",
    "Consecutive tests in the same class share one header",
)
...
@scenario(
    "features/glaze_rendering.feature",
    "New class prints a blank line then a new header",
)
...
@scenario(
    "features/glaze_rendering.feature",
    "Skipped test renders with SKIP badge and reason",
)
def test_skip_badge(): ...
...
@scenario(
    "features/glaze_rendering.feature",
    "Xfailed test renders with XFAIL badge and reason",
)
...
@scenario(
    "features/glaze_rendering.feature",
    "Xpassed test renders with XPASS badge and reason",
)
...
@scenario(
    "features/glaze_rendering.feature", "Teardown error renders after the passing test"
)
def test_teardown_error(): ...


@scenario(
    "features/glaze_rendering.feature", "Captured output is shown only on failures"
)
def test_captured_on_failure(): ...


@scenario(
    "features/glaze_rendering.feature", "Captured output is suppressed on passing tests"
)
...
@scenario(
    "features/glaze_rendering.feature", "FAIL badge and dash render in bright red"
)
def test_fail_color(): ...


@scenario(
    "features/glaze_rendering.feature", "ERROR badge and dash render in standard red"
)
...
@scenario(
    "features/glaze_rendering.feature", "XFAIL badge and dash render in bright red"
)
...
@scenario(
    "features/glaze_rendering.feature",
    "Received value in assertion renders in bright red",
)
def test_received_red(): ...


@scenario(
    "features/glaze_rendering.feature", "Expected value in assertion renders in green"
)
...
@given(
    parsers.parse('a failing test with assertion error "{error}"'),
...
@given(
    parsers.parse('a skipped test result with reason "{reason}"'),
    target_fixture="result",
)
...
@given(
    parsers.parse('an xfailed test result with reason "{reason}"'),
    target_fixture="result",
)
...
@given(
    parsers.parse('an xpassed test result with reason "{reason}"'),
    target_fixture="result",
)
...
@given(
    parsers.parse('a failing test result with captured stdout "{content}"'),
    target_fixture="result",
)
...
@given(
    parsers.parse('a passing test result with captured stdout "{content}"'),
    target_fixture="result",
)
...
@given(
    parsers.parse('a failing test with E lines containing "{noise}"'),
    target_fixture="result",
)
...
@given(
    parsers.parse("a passing test result with duration {duration:f} seconds"),
    target_fixture="result",
)
...
@given(
    parsers.parse('a failing test with a diff line "{diff_line}"'),
    target_fixture="result",
)
...
@given(
    parsers.parse('a failing test with a context E line "{context}"'),
    target_fixture="result",
)
...
@given(
    parsers.parse("{n:d} passing test results in the same file"),
...
@given(
    parsers.parse('2 passing test results with name "{name1}" and "{name2}"'),
    target_fixture="results",
)
...
@given(
    parsers.parse(
        "{n_pass:d} passing and {n_fail:d} failing test results in the same file"
    ),
    target_fixture="results",
)
...
@given(
    parsers.parse('a teardown error result "{error}"'), target_fixture="teardown_error"
)
...
def error_below_test(printed):
    ...
    e_idxs = [
        i for i, l in enumerate(printed) if strip_ansi(l).strip().startswith("E  ")
    ]
...
def assert_name_colorless(printed, name):
    result_lines = [
        l for l in printed if "---" in strip_ansi(l) and name in strip_ansi(l)
    ]
    assert result_lines, f"No result line containing '{name}' found"
    assert any(
        name_is_uncolored(l, name) for l in result_lines
